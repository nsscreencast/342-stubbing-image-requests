...
class CryptoCompareClient {
    ...
    func fetchPrices(fromSymbol: String, toSymbols: [String], completion: @escaping ApiCompletionBlock<PriceResponse>) {
        let url = URL(string: "https://min-api.cryptocompare.com/data/price")!
        var components = URLComponents(url: url, resolvingAgainstBaseURL: false)!
        components.queryItems = [
            URLQueryItem(name: "fsym", value: fromSymbol),
            URLQueryItem(name: "tsyms", value: toSymbols.joined(separator: ","))
        ]
            ...
            if let e = error {
                ...
                case 200:
                    ...
                    do {
                        ...
                        ApiResult.success(priceResponse) -=> completion
                    } catch let e {
                        debugPrint(e)
                        let bodyString = String(data: data!, encoding: .utf8)
                        ApiResult.failure(.responseFormatInvalid(bodyString ?? "<no body>")) -=> completion
