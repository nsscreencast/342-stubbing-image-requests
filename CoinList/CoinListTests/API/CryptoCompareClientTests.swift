...
class CryptoCompareClientTests : XCTestCase {
    ...
    func testFetchesPrices() {
        FixtureLoader.stubPrice(symbols: ["USD", "EUR"])
        let exp = expectation(description: "received response")
        client.fetchPrices(fromSymbol: "BTC", toSymbols: ["USD", "JPY", "EUR"], completion: { result in
            switch result {
            case .success(let priceResponse):
                XCTAssertEqual(priceResponse["USD"], 9914.74)
                XCTAssertEqual(priceResponse["EUR"], 8028.89)
            default:
                XCTFail()
            }
            exp.fulfill()
